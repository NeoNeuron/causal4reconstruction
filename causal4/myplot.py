...
def hist_causal_with_conn_mask(pm_causal:dict, hist_range:tuple=None)->None:
    ...
    fig.savefig('image/'+f'histogram_of_causal_with_conn_mask_{fname:s}.pdf')
...
def hist_dp(pm_causal:dict)->None:
    ...
    ax.set_xticklabels([r'$10^{%d}$'%val for val in xticks])
    ax.set_xlabel(r'$\Delta p_m$ or $\delta_p$ values')
    ax.set_ylabel('Probability density')
    plt.tight_layout()

    fig.savefig('image/'+f'histogram_of_dp_Delta_p_{fname:s}.pdf')
...
def hist_causal(pm_causal:dict, hist_range:tuple=None)->None:
    ...
    ax[0].set_xlim(xticks[0], xticks[-1])
    ax[0].set_xticklabels([r'$10^{%d}$'%val for val in xticks])
    ax[1].legend()
    ax[1].set_ylabel('True Positive Rate')
    ax[1].set_xlabel('True Negative Rate')
    ax[1].set_xticks([0,0.5,1])
    ...
    fig.savefig('image/'+f'histogram_of_causal_{fname:s}.pdf')