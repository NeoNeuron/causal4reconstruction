...
def get_sync_threshold(freq, N=100, binsize=2, T=1e3, dt=0.02):
    async_pop = bp.neurons.PoissonGroup(N, freqs=freq)
    runner = bp.DSRunner(async_pop, dt=dt, monitors=['spike'], progress_bar=False)
    runner.run(T)
    spike_data = np.asarray(np.nonzero(runner.mon['spike'])).astype(float)
    spike_data[0] = runner.mon.ts[spike_data[0].astype(int)]
    counts, bins = np.histogram(spike_data[0], bins=np.arange(0, T+binsize, binsize))
    return uniform_filter1d(counts, size=3).max()
...
def downsample(fname:str, ofname:str, path:str, ra:float, force_regen=False):
    ...
    print(f">> {100*del_spk_counts/total_spk_counts:.2f}% spikes have been deleted!")
#%%
def downsample_v2(fname:str, ofname:str, path:str, N:int, force_regen=False):
    """downsample spike train data
    ...
    with open(ifpath, 'rb') as f:
        ...
        while buff_len>0:
            ...
            spk_buff = np.array(unpack('d'*buff_len, buff_char)).reshape(-1,2)
            if threshold is None:
                firing_rate = spk_buff.shape[0]/spk_buff[-1,0]/N*1e3
                threshold = np.mean([get_sync_threshold(firing_rate, N, binsize) for _ in range(10)])
            ...
            if slices is not None and isinstance(slices, np.ndarray):
                spk_buff = np.vstack([slices, spk_buff])
            counts, _ = np.histogram(spk_buff[:,0], bins=edges)
            counts = uniform_filter1d(counts, size=3)
